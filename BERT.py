...
def run_BERT():
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(X, y)):
        ...
        for epoch in range(max_epochs):
            ...
            if val_loss < best_val_loss:
                ...
                if save_models:
                    torch.save(model.state_dict(), f"BERT_Models\\bert_{model_file_name}_best_model_fold_{fold + 1}.pt")
                print(
    ...
    results_df.to_csv(f"{pathlib.Path().resolve()}\\results\\BERT_{model_file_name}_Results.csv", index=False)
...
def evaluate_with_loaded_models():
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(X, y)):
        ...
        # Load saved model
        model_path = f"BERT_Models\\bert_{model_file_name}_best_model_fold_{fold + 1}.pt"
...
def bert_run_with_profiler():
    start_time = time.time()
    profile_output_filename = f'profile_outputs\\bert_{model_file_name}_profile_output'
    cProfile.run('run_BERT()', profile_output_filename)
    p = pstats.Stats(profile_output_filename)
    p.sort_stats(pstats.SortKey.TIME).print_stats(10)

    Notifier.notify_complete()

    print("--- BERT total duration: %s seconds ---" % (time.time() - start_time))

def bert_evaluate_run_with_profiler():
    start_time = time.time()
    profile_output_filename = f'profile_outputs\\bert_evaluate_{model_file_name}_profile_output'
    ...
    print("--- BERT evaluation total duration: %s seconds ---" % (time.time() - start_time))


# if __name__ == '__main__':
#     bert_evaluate_run_with_profiler()
